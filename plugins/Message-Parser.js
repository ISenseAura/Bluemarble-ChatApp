...
class MessageParser {
  constructor() {
    (this.prefix = "/"), (this.customs = {}), (this.type = null);
  }
  ...
  parseMessage(data) {
    ...
    this.type = null;
    if (msg.startsWith(this.prefix)) this.type = "command";
  ...
  parseCommand(data) {
    let msg = data.message;
    let arr = msg.split(" ");
    let cmd = arr[0].replace(this.prefix, "");
    ...
    delete user.pass;
    let output = Commands[cmd].command(user, targets, data);
    console.log(output);
    return output;
  }
}
